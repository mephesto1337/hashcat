...
KERNEL_FQ KERNEL_FA void m10902_loop (KERN_ATTR_TMPS_ESALT (pbewithsha256and256bitaes_cbc_bc_tmp_t, pbewithsha256and256bitaes_cbc_bc_t))
{
  ...
  u32x iv[16] = {0};
  u32x key[16] = {0};
  sha256_ctx_vector_t sha256_ctx;
  ...
  {
    sha256_init_vector(&sha256_ctx);
    sha256_update_vector(&sha256_ctx, iv, 32);
    sha256_final_vector(&sha256_ctx);
    for (u32 i = 0; i < 8; i++) {
      iv[i] = sha256_ctx.h[i];
    }

    sha256_init_vector(&sha256_ctx);
    sha256_update_vector(&sha256_ctx, key, 32);
    sha256_final_vector(&sha256_ctx);
    for (u32 i = 0; i < 8; i++) {
